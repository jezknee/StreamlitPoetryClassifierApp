...
def get_ai_bet(confidence, ai_chips):
    ...
    if confidence < 0.3:
        bet = max(1, ai_chips // 10)  # Low confidence: bet 10% or 1 chip
    elif confidence < 0.5:
        bet = max(1, ai_chips // 5)   # Medium-low confidence: bet 20%
    elif confidence < 0.7:
        bet = max(1, ai_chips // 3)   # Medium confidence: bet 33%
    else:
        bet = min(ai_chips, max(1, ai_chips // 2))  # High confidence: bet 50% or all
    ...
    if game_over:
        ...
        if st.session_state.answered:
            if not st.session_state.scored:
                ...
                st.write(f"**AI's prediction:** {predicted_author} (bet: {st.session_state.model_bet} chips)")
